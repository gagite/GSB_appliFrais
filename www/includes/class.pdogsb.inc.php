...
class PdoGsb {
    ...
    public function getInfosClassemnt() {
        $requetePrepare = PdoGsb::$monPdo->prepare(
                'SELECT  visiteur.nom AS nom, '
                . 'visiteur.prenom AS prenom, fichefrais.montantValide AS montantValide,  SUM( lignefraisforfait.quantite*fraisforfait.montant) AS quantite,  SUM(lignefraishorsforfait.montant) AS montant  '
                . 'FROM visiteur '
                ...
                . 'GROUP BY visiteur.nom, visiteur.prenom, fichefrais.montantValide '
                . 'ORDER BY fichefrais.montantValide '
        );
        $requetePrepare->execute();
        return $requetePrepare->fetchAll();
    }

    public function getmontantfraisforfais() {
        $requetePrepare = PdoGsb::$monPdo->prepare(
                'SELECT  lignefraisforfait.mois, SUM( lignefraisforfait.quantite*fraisforfait.montant) AS quantite'
                . 'FROM lignefraisforfait '
                . 'INNER JOIN fraisforfait '
                . 'ON lignefraisforfait.idfraisforfait = fraisforfait.id '
                . 'GROUP BY lignefraisforfait.mois '
        );
        $requetePrepare->execute();
        return $requetePrepare->fetchAll();
    }
     ...
     * @return null
     */
    /*     * public function majFraisHorsForfait($id,  $idVisiteur, $leMois, $libelle, $date, $montant) {
      $date1 = dateFrancaisVersAnglais($date);
      $requetePrepare = PdoGSB::$monPdo->prepare(
      ...
      $requetePrepare->bindParam(':uneDate', $date1, PDO::PARAM_STR);
      $requetePrepare->bindParam(':unLibelle', $libelle, PDO::PARAM_STR);
      $requetePrepare->bindParam(':unId',$id, PDO::PARAM_STR);
      $requetePrepare->execute();
      $requetePrepare1 = PdoGSB::$monPdo->prepare(

      'SELECT lignefraishorsforfait.montant as leMontant'
      ...
      $requetePrepare1->bindParam(':unMois', $leMois, PDO::PARAM_STR);
      $requetePrepare1->bindParam(':unLibelle', $libelle, PDO::PARAM_STR);
      $requetePrepare1->execute();
      $laLigne = $requetePrepare1->fetch();
      $leMontant =$laLigne['leMontant'];
      return$leMontant;


      }* */

    public function majFraisHorsForfait($idVisiteur, $leMois, $libelle, $date, $montant) {
    ...
    /**
     * modifie des frais déjà existant
     * 
     ...
     * @param type $montant
     */
    public function corrigerFraisForfait(
            $idVisiteur,
            $mois,
            $libelle,
            $date,
            $montant) {
        $requetePrepare = PdoGSB::$monPdo->prepare(
                'UPDATE INTO lignefraishorsforfait '
                . 'VALUES (null, :unIdVisiteur,:unMois, :unLibelle, :uneDateFr,'
                . ':unMontant) '
        );
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
    ...
    public function getChoixVisiteurs() {
        $requetePrepare = PdoGSB::$monPdo->prepare(
